...
bool SCRFD::Load(const std::string &model, const cv::Size &input_shape, const std::string &device) {
    ...
    if (!device.empty()) {
        ...
        if (0 != ret) {
            ...
            LOGD( "Set context device running in %s failed.\n", device.c_str());
            return false;
        }
    }
...
//    Timer timer;
    ...
    if (0 != ret) {
        ...
        LOGD( "Prerun graph failed(%d).", ret);
        return false;
    }
...
bool SCRFD::init_canvas(int width, int height, int channel) {
    ...
    if (TENGINE_DT_FP32 == tensor_type) {
        ret = set_tensor_buffer(input_tensor, this->canvas_float.data(),
                                (int) (this->canvas_float.size() * sizeof(float)));
        if (0 != ret) {
            fprintf(stderr, "Set input tensor buffer failed.\n");
            return false;
        }
    } else {
        ret = set_tensor_buffer(input_tensor, this->canvas_uint8.data(),
                                (int) (this->canvas_uint8.size() * sizeof(uint8_t)));
        if (0 != ret) {
            fprintf(stderr, "Set input tensor buffer failed.\n");
            return false;
        }
...
bool SCRFD::init_buffer() {
    ...
    for (int i = 0; i < this->strides.size(); i++) {
        ...
        if (TENGINE_DT_FP32 != tensor_type) {
            ...
            auto score_ret = get_tensor_quant_param(score_tensor, &(this->score_scale[i]),
                                                    &(this->score_zp[i]), 1);
            auto bbox_ret = get_tensor_quant_param(bbox_tensor, &(this->bbox_scale[i]),
                                                   &(this->bbox_zp[i]), 1);
            auto landmark_ret = get_tensor_quant_param(landmark_tensor, &(this->landmark_scale[i]),
                                                       &(this->landmark_zp[i]), 1);
            ...
            if (0 > landmark_ret) {
                fprintf(stderr,
                        "Get output landmark tensor(%s) quant param at stride(%d) failed.\n",
                        this->landmark_name[i].c_str(), this->strides[i]);
                return false;
            }
        }
    ...
    for (int i = 0; i < this->anchors.size(); i++) {
        ...
        for (int j = 0; j < this->ratios.size(); j++) {
            ...
            for (int k = 0; k < this->scales.size(); k++) {
                ...
                std::array<float, 4> point = {cx - rs_w * 0.5f, cx - rs_h * 0.5f, cx + rs_w * 0.5f,
                                              cx + rs_h * 0.5f};
...
bool SCRFD::pre(const cv::Mat &image) {
    cv::Mat image_resized(this->canvas_height, this->canvas_width, CV_8UC(image.channels()),
                          this->resized_container.data());
    letterbox(image, image_resized, cv::Scalar(0, 0, 0), this->canvas_width_gap,
              this->canvas_height_gap);
    ...
    cv::Mat image_resized_permute(this->canvas_height, this->canvas_width, CV_8UC(image.channels()),
                                  this->resized_permute_container.data());
    ...
    if (TENGINE_DT_FP32 == tensor_type) {
        ...
        for (int i = 0; i < image.channels(); i++) {
            ...
            for (int p = 0; p < planar_size; p++) {
                ...
                int val_round = (int) (std::round(
                        val / this->input_scale + (float) this->input_zp));
    ...
    for (int i = 0; i < this->strides.size(); i++) {
        ...
        if (this->is_quantization) {
            ...
            for (size_t j = 0; j < this->score_buffer[i].size(); j++) {
                this->score_buffer[i][j] = (float) ((int) score_uint8_ptr[j] - this->score_zp[i]) *
                                           this->score_scale[i];
            }

            for (size_t j = 0; j < this->bbox_buffer[i].size(); j++) {
                this->bbox_buffer[i][j] =
                        (float) ((int) bbox_uint8_ptr[j] - this->bbox_zp[i]) * this->bbox_scale[i];
            }

            for (size_t j = 0; j < this->landmark_buffer[i].size(); j++) {
                this->landmark_buffer[i][j] =
                        (float) ((int) landmark_uint8_ptr[j] - this->landmark_zp[i]) *
                        this->landmark_scale[i];
            }
        } else {
            ...
            for (int w = 0; w < fm_w; w++) {
                for (int c = 0; c < fm_c; c++) {
                    ...
                    if (score > this->score_threshold) {
                        ...
                        proposal.landmark[0].x = (cx +
                                                  this->landmark_buffer[i][landmark_offset + 0] *
                                                  (float) this->strides[i] -
                                                  this->canvas_width_gap) * this->input_ratio;
                        proposal.landmark[0].y = (cy +
                                                  this->landmark_buffer[i][landmark_offset + 1] *
                                                  (float) this->strides[i] -
                                                  this->canvas_height_gap) * this->input_ratio;
                        proposal.landmark[1].x = (cx +
                                                  this->landmark_buffer[i][landmark_offset + 2] *
                                                  (float) this->strides[i] -
                                                  this->canvas_width_gap) * this->input_ratio;
                        proposal.landmark[1].y = (cy +
                                                  this->landmark_buffer[i][landmark_offset + 3] *
                                                  (float) this->strides[i] -
                                                  this->canvas_height_gap) * this->input_ratio;
                        proposal.landmark[2].x = (cx +
                                                  this->landmark_buffer[i][landmark_offset + 4] *
                                                  (float) this->strides[i] -
                                                  this->canvas_width_gap) * this->input_ratio;
                        proposal.landmark[2].y = (cy +
                                                  this->landmark_buffer[i][landmark_offset + 5] *
                                                  (float) this->strides[i] -
                                                  this->canvas_height_gap) * this->input_ratio;
                        proposal.landmark[3].x = (cx +
                                                  this->landmark_buffer[i][landmark_offset + 6] *
                                                  (float) this->strides[i] -
                                                  this->canvas_width_gap) * this->input_ratio;
                        proposal.landmark[3].y = (cy +
                                                  this->landmark_buffer[i][landmark_offset + 7] *
                                                  (float) this->strides[i] -
                                                  this->canvas_height_gap) * this->input_ratio;
                        proposal.landmark[4].x = (cx +
                                                  this->landmark_buffer[i][landmark_offset + 8] *
                                                  (float) this->strides[i] -
                                                  this->canvas_width_gap) * this->input_ratio;
                        proposal.landmark[4].y = (cy +
                                                  this->landmark_buffer[i][landmark_offset + 9] *
                                                  (float) this->strides[i] -
                                                  this->canvas_height_gap) * this->input_ratio;
...
bool SCRFD::Detect(const cv::Mat &image, std::vector<region> &boxes, const float &score_th,
                   const float &nms_th) {
    if (nullptr == this->graph) {
