...
bool SCRFD::Load(const std::string& model, const cv::Size& input_shape, const std::string& device)
{
    ...
    if (!device.empty())
    {
        ...
        __android_log_print(ANDROID_LOG_DEBUG, "TenigneModel", "Set context device running in %s succeded.\n", device.c_str());

        if (0 != ret)
        {
            fprintf(stderr, "Set context device running in %s failed.\n", device.c_str());
            __android_log_print(ANDROID_LOG_DEBUG, "TenigneModel", "Set context device running in %s failed.\n", device.c_str());
            return false;
        }
    }
    ...
    __android_log_print(ANDROID_LOG_DEBUG, "TenigneModel", "Load model %s succeded.\n", model.c_str());

    if (nullptr == this->graph)
    {
        fprintf(stderr, "Load model %s failed.\n", model.c_str());
        __android_log_print(ANDROID_LOG_DEBUG, "TenigneModel", "Load model %s failed.\n", model.c_str());
        return false;
    }
...
//    Timer timer;
    ...
    if (0 != ret)
    {
        ...
        __android_log_print(ANDROID_LOG_DEBUG, "TenigneModel", "Prerun graph failed(%d).", ret);
        return false;
    }
    ...
    if (!buffer_ret)
    {
        ...
        __android_log_print(ANDROID_LOG_DEBUG, "TenigneModel", "Init buffer failed.\n");
        return false;
    }
    ...
    if (!anchor_ret)
    {
        ...
        __android_log_print(ANDROID_LOG_DEBUG, "TenigneModel", "Init anchors failed.\n");
        return false;
    }
